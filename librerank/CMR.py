...
class CMR_generator(RLModel):
    ...
    def deep_set_encode(self):
        ...
        self.enc_input = tf.concat([self.itm_enc_input, tf.tile(self.usr_enc_input, [1, self.item_size, 1])], axis=-1)
    ...
    def _build_loss(self):
        # self.lr = 1e-4
        self.gamma = 1
        with tf.variable_scope("cmr_train_input"):
        ...
        act_idx_one_hot = tf.one_hot(indices=self.actions, depth=self.item_size)  # [B*N, N]
