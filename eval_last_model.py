...
def eval_last(model, data, batch_size, isrank, metric_scope, _print=False, evaluator=None):
    ...
    for batch_no in range(batch_num):
        ...
        for i in range(step_sizes):
            ...
            div_metrics = res[-1][-1]
            # batch_ave = np.array(batch_ave)*0+np.array(div_metrics)*1
...
def reranker_parse_args():
    ...
    parser.add_argument('--reload_path', type=str,
                        default='./model//save_model_ad/10/202303231806_lambdaMART_LAST_generator_16_0.0001_0.0001_64_16_0.8_1',
                        help='model ckpt dir')
